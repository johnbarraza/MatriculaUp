...
class MatriculaApp:
    ...
    def get_sections_for_course(self, course_name: str) -> List[str]:
        ...
        for idx, r in rows.iterrows():
            ...
            # Formato más limpio: Secc A — Profesor — Tipo — Día HH:MM-HH:MM
            label = f"{course_name}__{secc}|Secc {secc} — {prof} — {tipo} — {dia} {ini}-{fin} ({cred} cr)"
            opts.append(label)
        ...
        # Guardar en caché
        self._sections_cache[course_name] = unique
    ...
    def add_to_schedule(self, course_labels: List[str], schedule_index: int,
        ...
        # Construir lista de nuevas filas a añadir
        new_rows = []
        for label in course_labels:
            ...
            if not rows.empty:
                ...
                new_rows.append({
                    ...
                    'tipo': first.get('Tipo', ''),
                    'dia': first.get('Día', ''),
                    'inicio': first.get('Horario_Inicio', ''),
                    'fin': first.get('Horario_Cierre', ''),
                    'cred': cred_val
                })
    ...
    def _detect_conflicts_with_new(self, schedule_index: int,
        ...
        for nr in new_rows:
            ...
            tipo_n = (nr.get('tipo') or '').upper().strip()

            for eb in existing:
                tipo_e = (eb.get('tipo') or '').upper().strip()

                # Solo comparar si son del mismo grupo (clases vs clases, exámenes vs exámenes)
                same_group = (
                    (tipo_n in self.CLASES_SET and tipo_e in self.CLASES_SET) or
                    (tipo_n in self.EXAMENES_SET and tipo_e in self.EXAMENES_SET)
                )
                ...
                # Verificar mismo día
                if str(nr.get('dia')).upper().strip() != str(eb.get('dia')).upper().strip():
                    continue

                inicio_e = eb.get('inicio')
                fin_e = eb.get('fin')
    ...
    def detect_conflicts(self, schedule_index: int) -> Tuple[List, str]:
        ...
        for i in range(len(rows)):
            for j in range(i+1, len(rows)):
                ...
                # Verificar mismo día
                if str(a.get('dia')).upper().strip() != str(b.get('dia')).upper().strip():
                    continue

                tipo_a = (a.get('tipo') or '').upper().strip()
                tipo_b = (b.get('tipo') or '').upper().strip()

                # Solo comparar si son del mismo grupo
                same_group = (
                    (tipo_a in self.CLASES_SET and tipo_b in self.CLASES_SET) or
                    (tipo_a in self.EXAMENES_SET and tipo_b in self.EXAMENES_SET)
                )

                if not same_group:
                    continue

                inicio_a = a.get('inicio')
                fin_a = a.get('fin')
                inicio_b = b.get('inicio')
                fin_b = b.get('fin')
                ...
                try:
                    sa = datetime.strptime(str(inicio_a), '%H:%M')
                    ea = datetime.strptime(str(fin_a), '%H:%M')
                    sb = datetime.strptime(str(inicio_b), '%H:%M')
                    eb = datetime.strptime(str(fin_b), '%H:%M')
                except:
                    continue
                ...
                if sa < eb and ea > sb:
                    conflicts.append((
                        a['block'], b['block'], a.get('dia'),
                        a.get('inicio'), a.get('fin'),
                        b.get('inicio'), b.get('fin')
                    ))
    ...
    def draw_week_schedule(self, schedule_index: int, save_path: Optional[str] = None) -> Tuple[Image.Image, Image.Image]:
        """
        Dibuja horario semanal mejorado (7:30 AM - 11:30 PM) con caché.
        Solo regenera si el horario cambió desde la última vez.
        """
        rows = self.schedules.get(schedule_index, [])

        # Generar hash del contenido del horario para verificar cambios
        schedule_hash = str(sorted([f"{r['block']}_{r['dia']}_{r['inicio']}_{r['fin']}" for r in rows]))
        ...
        end_time = datetime.strptime('23:30', '%H:%M')  # 11:30 PM
        ...
        def make_fig(filter_types: str) -> Image.Image:
            ...
            # Dibujar bloques de cursos con estilo mejorado
            for r in rows:
                tipo = (r.get('tipo') or '').upper()

                # Filtrar por tipo
                if filter_types == 'CLASE' and tipo not in self.CLASES_SET:
                    continue
                if filter_types == 'EXAM' and tipo not in self.EXAMENES_SET:
                    continue

                dia = str(r.get('dia') or '').upper()
                ...
                if filter_types == 'CLASE':
                    if overlap:
                        face_color = '#ff6b6b'  # Rojo para conflictos
                        edge_color = '#c92a2a'
                    else:
                        face_color = '#4dabf7'  # Azul moderno
                        edge_color = '#1971c2'
                else:  # EXAM
                    if overlap:
                        face_color = '#ff8787'
                        ...
                        face_color = '#ffd43b'  # Amarillo cálido
                        edge_color = '#f59f00'
    ...
    def _check_overlap_in_schedule(self, current: dict, all_rows: List[dict],
        ...
        inicio_curr = current.get('inicio')
        fin_curr = current.get('fin')

        if not inicio_curr or not fin_curr:
            return False

        try:
            st = datetime.strptime(str(inicio_curr), '%H:%M')
            en = datetime.strptime(str(fin_curr), '%H:%M')
        except:
            return False

        dia = str(current.get('dia')).upper().strip()
        tipo = (current.get('tipo') or '').upper()

        for other in all_rows:
            if other is current:
                continue

            if str(other.get('dia')).upper().strip() != dia:
                continue

            otipo = (other.get('tipo') or '').upper()

            # Verificar mismo grupo
            if filter_types == 'CLASE':
                if tipo not in self.CLASES_SET or otipo not in self.CLASES_SET:
                    continue
            elif filter_types == 'EXAM':
                if tipo not in self.EXAMENES_SET or otipo not in self.EXAMENES_SET:
                    continue

            inicio_other = other.get('inicio')
            fin_other = other.get('fin')

            if not inicio_other or not fin_other:
                continue

            try:
                ost = datetime.strptime(str(inicio_other), '%H:%M')
                oen = datetime.strptime(str(fin_other), '%H:%M')
            except:
                continue
    ...
    with gr.Blocks(title="MatriculaUP - Planificador de Horarios") as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                gr.Markdown("---")
                gr.Markdown("### 📊 Vista Semanal (7:30 AM - 11:30 PM)")
        ...
        def blocks_choices(idx: int) -> List[str]:
            ...
            return [b['block'] for b in app_logic.schedules.get(idx, [])]
        ...
        btn_load.click(
            handle_load_data,
            inputs=[upload],
            outputs=[status, course_dropdown, remove_dd1, remove_dd2, remove_dd3, classes_img, exams_img]
        )
        ...
        def on_course_select(course):
            ...
            return gr.update(choices=secs, value=[])
